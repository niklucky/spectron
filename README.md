...
Project settings includes **Issue types** and **Tags** dictionaries. Each issue has one optional type and any number of shared project tags, stored through an issue–tag junction table. Existing and new projects start with Story, Epic, Bug and Task types. Manage the dictionaries in settings and assign them in the issue editor; changes appear in issue history.

In Jira's **Field mapping** tab, enable **Jira labels → Tags** to match labels by name and create missing tags during import. An imported label list replaces the issue's tag assignments; an empty list clears them. Explicit issue publishing sends changed tag assignments back as Jira labels. Map Jira issue types to local types separately. These mappings are opt-in, and unmapped classifications are preserved. Tags and types use the existing sync conflict checks.
...
1. In **Connection**, enter the `https://your-team.atlassian.net` site, project key, account email and an **unscoped API token**. Test the connection and save credentials. Choose the default issue type in **Field mapping**.
2. Open Field mapping. Saved mappings appear immediately and Jira options refresh automatically without changing them. Expand Statuses, Priorities, Issue types, Fields or Users; each section shows mapped/total counts. Click Edit to change mappings, then Save to lock the form again. Cancel restores saved values. Optionally link Jira users to project members and map priorities to existing local options. Create local fields in the Fields tab, then map Jira fields to them; leave unsupported or unwanted fields ignored. Save mappings before importing. Missing users, including historical authors, are created automatically in `external_identities`; they have no login, fake email or membership. The External users section lets an owner link or unlink an identity to an existing project member, updating displayed attribution without moving or duplicating records.
...
4. Use **Create in Jira / Update Jira issue** on an issue, or **Send comment to Jira** on a comment. These owner-only actions remain available for explicit publishing. Automatic export can be enabled separately in Sync → Export. Jira workflow transitions enforce status changes. Remote comments are written using the connected Jira account and its permissions.

Credentials and the automatic import schedule also require Edit before changes and lock after Save. Full import uses saved mappings even while a mapping draft is open.
...
### Automatic export to Jira and Yandex Tracker

Sync → Export supports Off, On save, or By schedule (15 minutes, hourly, daily), with independent switches for issue creation/updates, comment creation/updates, and worklog creation/deletion. Settings require Edit and lock after Save. A persistent queue and export log handle retries, dependencies and uncertain creates. Export remains off by default. See [automatic export setup and recovery](docs/export-sync.md).
