      ...
      <main className="mx-auto max-w-[1080px] px-6 pb-24">
        ...
        <Section id="list" title="Issue list" lead="Each row is a conversation: who started it and where, then the key line, the title, and who spoke last. Unread rows are bold.">
          <div className="max-w-[340px] rounded-xl bg-surface p-2 hairline">
            ...
            <IssueRow active unread={3} author={people.nikita} project={project} statusTrigger="started" issueKey="SPC-142" time="11:04" title="Agent runs: retries duplicate draft PRs when push times out" preview={<><Avatar name={agents.reviewer.name} kind="agent" size="sm" working /><span className="truncate"><b>Reviewer</b> is reviewing PR #218</span></>} />
            <IssueRow unread={1} author={people.marina} project={{ name: "Website", initial: "W", logo: null }} statusTrigger="started" issueKey="WEB-31" time="10:52" title="Pricing page: annual toggle resets on locale switch" preview={<><Avatar name={agents.dev.name} kind="agent" size="sm" /><span className="truncate"><b>Dev senior</b> <span className="font-medium text-warn">needs your input</span></span></>} />
            <ListGroup>Today</ListGroup>
            <IssueRow author={people.nikita} project={project} statusTrigger="review" issueKey="SPC-139" time="09:47" title="Jira sync: comments published twice after reconnect" preview={<><Avatar name={people.marina.name} size="sm" /><span className="truncate"><b>Marina:</b> Merged. Closing once the nightly passes.</span></>} />
            <IssueRow author={people.ilya} project={project} statusTrigger="finished" issueKey="SPC-133" time="Tue" title="Stop button leaves container running after cancel" preview={<><Avatar name={agents.dev.name} kind="agent" size="sm" /><span className="truncate"><b>Dev senior:</b> Fixed and merged in #209.</span></>} />
          </div>
        </Section>
