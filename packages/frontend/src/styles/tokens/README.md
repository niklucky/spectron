...
The CSS files in this directory are the source of truth. Their values preserve the approved blue-gray design. They are plain CSS custom properties, usable by any web framework.
...
| Group | Tokens |
| --- | --- |
| Surfaces | `--color-surface`, `--color-sidebar`, `--color-list`, `--color-menu` |
...
| Conversation | `--color-bubble`, `--color-own-bubble`, `--color-line` |
...
Light values live in `:root`; dark overrides live in `:root[data-theme="dark"]`. Identity, status, and media colors currently share values across both themes. The public page retains its neutral foreground/background through `--color-page-text` and `--color-page-background`.
