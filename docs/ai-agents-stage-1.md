...
- `pnpm test:ai`: 12 passing tests, including encryption/provider-boundary and authenticated API tests against a newly created isolated PostgreSQL database. Covers owner isolation, CSRF, key replacement, concurrent check/replacement, sharing across projects, revoked membership, deleted identities, avatars, all catalog defaults, and unsupported efforts. Provider calls are mocked; no real API keys or paid calls were used.
