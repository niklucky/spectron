...
`git/workflow.ts` provides the durable service. The server polls due workspaces in small batches every five seconds; successful open requests become due after 60 seconds, closed/merged requests after ten minutes. Refresh is explicit; a spinner corresponds to a real active sync lease. Deleted issues and archived projects are excluded from scheduled work.
...
- Full agent suite with `TEST_AGENT_DOCKER=true pnpm test:agent-runs`: 44 passed, zero skipped, before the final ready-to-draft regression was added. The final focused provider/workflow suite passed all ten tests, including that added regression (45 distinct agent-suite tests total).
