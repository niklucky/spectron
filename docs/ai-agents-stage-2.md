...
- Private-network GitLab requires the server administrator to set `GITLAB_ALLOWED_PRIVATE_ORIGINS`, a comma-separated list of exact HTTPS origins, for example `https://gitlab.internal.example,https://gitlab.internal.example:8443`. Entries are normalized to origins, accepting mixed-case hosts, trailing slashes, and installation paths; paths do not narrow the allowlist scope. Invalid URLs, non-HTTPS URLs, credentials, query parameters, and fragments fail startup with a configuration error. Other private/loopback destinations are blocked. Do not add untrusted origins.
...
- PR review fixes: tolerate a failed or empty DNS address family and abort pending siblings after usable results; normalize and validate private-origin configuration; retain the last known actor on failed checks while preserving access gates and token-replacement resets. Focused DNS/configuration tests and API failure-state regressions cover these cases. All 20 Git/project-logo tests, workspace typechecks, and both builds passed.
